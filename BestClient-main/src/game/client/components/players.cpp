...
void CPlayers::RenderHookCollLine(
        ...
        const CNetObj_Character *pPlayerChar,
        int ClientId,
        const vec2 *pOverridePosition,
        const vec2 *pOverrideDirection, const float *pOverrideLength)
{
        const bool OverrideDeadTee =
                pOverridePosition != nullptr &&
                pOverrideDirection != nullptr;

        // The custom freeze laser replaces the regular hook collision line
        // for frozen/dead tees. Avoid drawing the duplicate blue line.
        if(!OverrideDeadTee &&
                g_Config.m_BcFreezeLaser &&
                in_range(ClientId, MAX_CLIENTS - 1))
        {
                const auto &HookClientData = GameClient()->m_aClients[ClientId];

                if(HookClientData.m_FreezeEnd != 0 ||
                        HookClientData.m_LiveFrozen ||
                        HookClientData.m_DeepFrozen)
                {
                        return;
                }
        }
	...
	if(!OverrideDeadTee && IVideo::Current() && !g_Config.m_ClVideoShowHookCollOther && !Local)
		return;
#endif
		...
		Client()->State() != IClient::STATE_DEMOPLAYBACK)
	{
	  ...
	  if(OverrideDeadTee)
        {
       AlwaysRenderHookColl = false;
	    RenderHookCollPlayer = true;
        }
        ...
        if(OverrideDeadTee)
        {
                Direction = normalize(*pOverrideDirection);
                Position = *pOverridePosition;
        }
        else
        {
	...
	static constexpr float HOOK_START_DISTANCE = CCharacterCore::PhysicalSize() * 1.5f;
	                const int TuningClientId = OverrideDeadTee ? GameClient()->m_Snap.m_LocalClientId : ClientId;

        float HookLength = pOverrideLength != nullptr ? *pOverrideLength : (float)GameClient()->m_aClients[TuningClientId].m_Predicted.m_Tuning.m_HookLength;
        float HookFireSpeed = (float)GameClient()->m_aClients[TuningClientId].m_Predicted.m_Tuning.m_HookFireSpeed;

        const bool CustomWithinHookRange =
                OverrideDeadTee &&
                pOverrideLength != nullptr &&
                *pOverrideLength <= (float)GameClient()->m_aClients[TuningClientId].m_Predicted.m_Tuning.m_HookLength;
        int CustomCollisionState = 0;
        const int HookOwnerClientId = OverrideDeadTee ? GameClient()->m_Snap.m_LocalClientId : ClientId;

        if(OverrideDeadTee &&
                in_range(ClientId, MAX_CLIENTS - 1) &&
                in_range(HookOwnerClientId, MAX_CLIENTS - 1))
        {
                const auto &OwnClientData = GameClient()->m_aClients[HookOwnerClientId];
                const auto &TargetClientData = GameClient()->m_aClients[ClientId];

                const bool IsOneSuper = TargetClientData.m_Super || OwnClientData.m_Super;
                const bool IsOneSolo = TargetClientData.m_Solo || OwnClientData.m_Solo;

                const bool TargetHookable =
                        IsOneSuper ||
                        (GameClient()->m_Teams.SameTeam(HookOwnerClientId, ClientId) &&
                                !IsOneSolo &&
                                !OwnClientData.m_HookHitDisabled);

                CustomCollisionState = TargetHookable ? 1 : 2;
        }

	// janky physics
	if(!OverrideDeadTee && (HookLength < HOOK_START_DISTANCE || HookFireSpeed <= 0.0f))
		return;

	vec2 QuantizedDirection = Direction;
	vec2 StartOffset = OverrideDeadTee ? vec2(0.0f, 0.0f) : Direction * HOOK_START_DISTANCE;
	vec2 BasePos = Position;
	vec2 LineStartPos = BasePos + StartOffset;
	vec2 SegmentStartPos = LineStartPos;

	ColorRGBA HookCollColor = color_cast<ColorRGBA>(ColorHSLA(OverrideDeadTee ? g_Config.m_BcFreezeLaserColor : g_Config.m_ClHookCollColorNoColl));
	...
	auto AddHookPlayerSegment = [&](const vec2 &StartPos, const vec2 &EndPos, const vec2 &HookablePlayerPosition, const vec2 &HitPos) {
                if(OverrideDeadTee)
                        if(CustomCollisionState != 2)
                                CustomCollisionState = 1;
                HookCollColor = color_cast<ColorRGBA>(ColorHSLA(OverrideDeadTee ? g_Config.m_BcFreezeLaserHookableColor : g_Config.m_ClHookCollColorTeeColl));
	...
	for(HookTick = 0; HookTick < MaxHookTicks; ++HookTick)
	{
		...
		if(distance(BasePos, SegmentEndPos) > HookLength)
		{
			...
			if(!HookEnteredTelehook)
			{
				...
				if(GameClient()->IntersectCharacter(SegmentStartPos, HitPos, RetractingHookEndPos, HookOwnerClientId, &IntersectedPlayerPosition) != -1)
				{
		...
		if(GameClient()->IntersectCharacter(SegmentStartPos, HitPos, SegmentEndPos, HookOwnerClientId, &IntersectedPlayerPosition) != -1)
		{
		...
		{
                if(OverrideDeadTee)
                        CustomCollisionState = 2;
			if(OverrideDeadTee)
                                HookCollColor = color_cast<ColorRGBA>(ColorHSLA(Hit == TILE_NOHOOK ? g_Config.m_BcFreezeLaserUnhookableColor : g_Config.m_BcFreezeLaserHookableColor));
                        else if(Hit != TILE_NOHOOK)
                                HookCollColor = OverrideDeadTee ? color_cast<ColorRGBA>(ColorHSLA(g_Config.m_BcFreezeLaserHookableColor)) : color_cast<ColorRGBA>(ColorHSLA(g_Config.m_ClHookCollColorHookableColl));
	...
	vLineSegments.emplace_back(Position, Position + StartOffset);
        ...
        if(OverrideDeadTee && CustomWithinHookRange)
        {
                std::vector<IGraphics::CLineItem> DashedSegments;
                constexpr float DashLength = 18.0f;
                constexpr float GapLength = 12.0f;

                for(const auto &Segment : vLineSegments)
                {
                        const vec2 Start(Segment.m_X0, Segment.m_Y0);
                        const vec2 End(Segment.m_X1, Segment.m_Y1);
                        ...
                        const vec2 SegmentDirection = Delta / SegmentLength;

                        for(float Offset = 0.0f; Offset < SegmentLength; Offset += DashLength + GapLength)
                        {
                                const float DashEnd = minimum(Offset + DashLength, SegmentLength);
                                DashedSegments.emplace_back(
                                        Start + SegmentDirection * Offset,
                                        Start + SegmentDirection * DashEnd);
                        }
                }

                vLineSegments = DashedSegments;
        }
        ...
        if(AlwaysRenderHookColl && RenderHookCollPlayer)
	{
		...
		HookCollColor = color_invert(HookCollColor);
                ...
                // Custom final hook laser color.
        if(OverrideDeadTee)
        {
                if(CustomCollisionState == 2)

                        HookCollColor = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_BcFreezeLaserUnhookableColor));

                else if(!CustomWithinHookRange)

                        HookCollColor = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_BcFreezeLaserColor));

                else

                        HookCollColor = color_cast<ColorRGBA>(ColorHSLA(g_Config.m_BcFreezeLaserHookableColor));
        }

        // Render hook coll line
	const int HookCollSize = OverrideDeadTee ? g_Config.m_BcFreezeLaserWidth : (Local ? g_Config.m_ClHookCollSize : g_Config.m_ClHookCollSizeOther);
	...
	Alpha *= (float)g_Config.m_ClHookCollAlpha / 100;
        if(OverrideDeadTee)
                Alpha *= (float)g_Config.m_BcFreezeLaserAlpha / 100.0f;
...
void CPlayers::RenderTeeLines()
{
        if(!g_Config.m_ClTeeLine)
                return;

        const int LocalClientId = GameClient()->m_Snap.m_LocalClientId;
        if(!in_range(LocalClientId, MAX_CLIENTS - 1))
                return;

        if(!IsPlayerInfoAvailable(LocalClientId))
                return;

        const vec2 LocalPosition = GameClient()->m_aClients[LocalClientId].m_RenderPos;
        const float MaxDistance = static_cast<float>(g_Config.m_ClTeeLineDistance) * 32.0f;

        std::vector<IGraphics::CLineItem> vLines;
        vLines.reserve(MAX_CLIENTS);
        ...
        for(int ClientId = 0; ClientId < MAX_CLIENTS; ++ClientId)
        {
                ...
                const bool IsAvailable = IsPlayerInfoAvailable(ClientId);

                if(IsAvailable)
                {
                        if(m_aTeeWasInactive[ClientId])
                        {
                                m_aTeeWasInactive[ClientId] = false;
                                m_aDeadTee[ClientId] = false;
                                m_aLastTeePosValid[ClientId] = false;
                        }

                        if(!m_aDeadTee[ClientId])
                        {
                                m_aLastTeePos[ClientId] = GameClient()->m_aClients[ClientId].m_RenderPos;
                                m_aLastTeePosValid[ClientId] = true;
                        }
                }
        else
        {
                m_aTeeWasInactive[ClientId] = true;
                m_aDeadTee[ClientId] = false;
                m_aLastTeePosValid[ClientId] = false;
        }

                if(!m_aDeadTee[ClientId] || !m_aLastTeePosValid[ClientId])
                        continue;

                const vec2 TeePosition = m_aLastTeePos[ClientId];

                if(distance(LocalPosition, TeePosition) > MaxDistance)
                        continue;

                vLines.emplace_back(LocalPosition, TeePosition);
        }
        ...
        Graphics()->TextureClear();
        Graphics()->LinesBegin();
        Graphics()->SetColor(0.20f, 0.80f, 1.00f, 0.70f);
        Graphics()->LinesDraw(vLines.data(), static_cast<int>(vLines.size()));
        Graphics()->LinesEnd();
        Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
}
void CPlayers::OnRender()
{
	...
	for(const auto &Client : GameClient()->m_aClients)
	{
        ...
        {
                const int DeadLaserLocalClientId = GameClient()->m_Snap.m_LocalClientId;

                if(g_Config.m_BcFreezeLaser &&
                        in_range(DeadLaserLocalClientId, MAX_CLIENTS - 1) &&
                        IsPlayerInfoAvailable(DeadLaserLocalClientId))
                {
                        const vec2 LocalPosition =
                                GameClient()->m_aClients[DeadLaserLocalClientId].m_RenderPos;

                        for(int DeadClientId = 0; DeadClientId < MAX_CLIENTS; ++DeadClientId)
                        {
                                if(DeadClientId == DeadLaserLocalClientId)
                                        continue;



                                                                                                if(!IsPlayerInfoAvailable(DeadClientId))
                                        continue;

                                const auto &FrozenClient = GameClient()->m_aClients[DeadClientId];
                                const bool IsFrozen =
                                        FrozenClient.m_FreezeEnd != 0 ||
                                        FrozenClient.m_LiveFrozen ||
                                        FrozenClient.m_DeepFrozen;

                                if(!IsFrozen)
                                        continue;

                                const vec2 DeadPosition = FrozenClient.m_RenderPos;
                                const vec2 DeadDirection = DeadPosition - LocalPosition;

                                const float DistanceToFreezeTee = length(DeadDirection);
                                const float NormalHookLength = (float)GameClient()->m_aClients[DeadLaserLocalClientId].m_Predicted.m_Tuning.m_HookLength;
                                const float DesiredHookLength = std::min(DistanceToFreezeTee, (float)g_Config.m_BcFreezeLaserMaxDistance);

                                if(length(DeadDirection) <= 0.01f)
                                        continue;

                                RenderHookCollLine(
                                        &GameClient()->m_aClients[DeadClientId].m_RenderPrev,
                                        &GameClient()->m_aClients[DeadClientId].m_RenderCur,
                                        DeadClientId,
                                        &LocalPosition,
                                        &DeadDirection, &DesiredHookLength);
                        }
                }
        }
...
void CPlayers::MarkTeeDead(int ClientId)
{
        if(!in_range(ClientId, MAX_CLIENTS - 1))
                return;

        m_aDeadTee[ClientId] = true;

        if(!m_aLastTeePosValid[ClientId])
        {
                m_aLastTeePos[ClientId] = GameClient()->m_aClients[ClientId].m_RenderPos;
                m_aLastTeePosValid[ClientId] = true;
        }
